...
export function registerCreateSubgraphCommand(
  ...
  const createSubgraph = () => {
    ...
    const subgraphTemplate = reactive<CodeGraphTemplate>(
      new CodeGraphTemplate(
        {
    ...
    ) as CodeGraphTemplate;

    editor.addGraphTemplate(subgraphTemplate);
    const nt = editor.nodeTypes.get(getGraphNodeTypeString(subgraphTemplate));
    if (!nt) throw new Error("Unable to create subgraph: Could not find corresponding graph node type");
    ...
    if (handler.canExecuteCommand(Commands.SAVE_SUBGRAPH_COMMAND))
      handler.executeCommand<SaveSubgraphCommand>(Commands.SAVE_SUBGRAPH_COMMAND);
